...
### Penetration
A universal stat (from Perks or equipment affixes) that reduces the defender's effective Soak before the Soak formula. Subject to its own **diminishing returns**: `Effective_Pen = Pen × K_pen / (Pen + K_pen)`. Then: `Effective Soak = Soak[family] - Effective_Pen` (minimum 0). Same harmonic formula shape as Soak and Resistance. One value — applies against all damage families equally. Does NOT affect Shields. K_pen is a tuning constant. See [combat.md](domains/combat.md).
...
Within a single Action or Trigger, effect components resolve in type order: (0) Revive effects — resolve first, before all other effects; (1) status/buff/debuff effects; (2) damage/healing effects; (3) movement effects. Components within the same category resolve simultaneously. Revive at Step 0 ensures hybrid revive+buff+heal Actions work: Revive clears all statuses (clean slate), then fresh buffs apply, then heals apply. Defined authoritatively by the [combat](combat.md) spec. Multi-Trigger resolution remains simultaneous (all qualifying Triggers fire together).
