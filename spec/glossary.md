...
### Might
Primary Attribute. Melee damage, equipment requirements (heavy), forced movement, physical intimidation. Also **mitigates armor Speed penalties** via `effective_penalty = base_penalty × (1 - Might/(Might+K_armor))` — strong fighters bear heavy gear more effectively.
...
An active combat ability granted by a Perk. Used on the character's turn during combat. Has an Action Speed, optional cooldown, optional resource cost, and tag-based targeting (e.g., `[Enemy, Single]`, `[Ally, AoE-Zone]`). All cooldowns reset fully between fights (per-combat only — no cross-fight persistence). Outputs are expressed as an effect component list — an ordered list of typed effect components (Damage, Heal, ApplyStatus, etc.) that resolve simultaneously. See [traits-and-perks.md](domains/traits-and-perks/index.md) for the full model.
...
A conditional automatic effect from a Perk that fires when specific combat events occur (e.g., OnHit, OnHitBy, OnAllyFallen, OnTurnStart). Not activated by the player or AI — resolves automatically. Uses the same effect component list as Actions for its outputs. **Triggers can chain** — a Trigger's effect can fire other Triggers. **Recursion guard**: each individual Trigger instance fires at most once per originating event (implementation: per-event "fired set"). **Three timing categories**: (1) immediate triggers fire during Action resolution (OnHit, OnDamageDealt, OnBlock, OnDodge, OnHeal, OnHealedBy, OnSummon, etc.), (2) revive triggers fire during Step 0 of effect resolution (OnRevive), (3) deferred kill triggers fire in tick Step 5 after Fallen Resolution (OnKill, OnFallen, OnAllyFallen — confirmed kills only, not false kills from healed targets). 30+ event types total (OnOverkill removed — overkill data available in OnKill payload). The canonical Trigger event vocabulary is owned by the combat spec.
...
### Tick (Combat)
A single time step in combat resolution — the atomic time unit. Each tick resolves in fixed order: (1) Effects Phase — per-tick status effects resolve (internal order: decay → effects → shield expiry); (2) Initiative Phase — all characters accumulate Initiative; (3) Turns Phase — characters with Initiative ≥ 100 act (one turn per tick maximum); (4) Fallen Resolution — characters at ≤0 HP enter Fallen state; (5) Kill Triggers — deferred kill-related triggers (OnKill, OnFallen, OnAllyFallen) fire as a batch. Deaths from any source are deferred to end of tick. See [combat.md](domains/combat/index.md) for full tick resolution order.
...
### Action Speed
A flat modifier to the base −100 Initiative cost after acting. Positive values = faster recovery (Action Speed +30 → only −70 Initiative cost). Negative values = longer delay (Action Speed −40 → −140 Initiative cost). Default Action Speeds: Attack = 0 (−100), Move = +25 (−75), Defend = +25 (−75), Search = +50 (−50). Non-offensive default Actions are faster than Attack to reward tactical decisions.
...
### Soak
Per-damage-type damage reduction stat used in step 6 of the per-component damage pipeline. Uses **three family-level base formulas**: Physical Soak (60% Endurance + 25% Might + 15% Speed), Elemental Soak (45% Endurance + 35% Awareness + 20% Luck), Magical Soak (55% Willpower + 25% Intellect + 20% Luck). Each damage type uses its family's Soak value. Formula: `Reduction % = Soak[family] / (Soak[family] + K)` where K is a global tuning constant. Per-type bonuses via tag-scoped Stat Adjustments (e.g., "+5 Soak vs [Fire]") add to the family base for specific types. Penetration (with diminishing returns) reduces effective Soak before the formula: `Effective Soak = Soak[family] - Effective_Pen` (minimum 0), where `Effective_Pen = Pen × K_pen / (Pen + K_pen)`.
...
### Penetration
A universal stat (from Perks or equipment affixes) that reduces the defender's effective Soak before the Soak formula. Subject to its own **diminishing returns**: `Effective_Pen = Pen × K_pen / (Pen + K_pen)`. Then: `Effective Soak = Soak[family] - Effective_Pen` (minimum 0). Same harmonic formula shape as Soak and Resistance. One value — applies against all damage families equally. Does NOT affect Shields. K_pen is a tuning constant. See [combat.md](domains/combat/index.md).
...
### Status Effect
A temporary condition applied to a character, tracked as stacks with a per-type decay formula. Unified system for buffs, debuffs, DoTs, and crowd control. **Additive stacking**: new applications ADD stacks to existing stacks. **Stacks are duration**: no separate duration tracking — decay removes stacks over time. Each status type defines its own **decay formula** (percentage-based like `current_stacks × 0.25`, flat, or attribute-derived like `Willpower/10`). **Ceiling rounding on decay removal** makes statuses shorter-lived: Burning 25% decay = `ceil(10×0.25)=3` stacks removed → 10→7→4→2→1→0 (5 ticks, not 8). Character status tracking is minimal: `{status_type, current_stacks}`. Max stacks effectively unlimited (9999 cap). **Minimum 1 stack**: status effects always apply at least 1 stack even after Resistance reduction. **No true immunity**: defense against powerful statuses comes from high Resistance (reduces to 1 stack) + fast decay (clears quickly), not binary immunity Perks. DoT damage bypasses Soak AND Block (DoT ticks occur in Effects Phase, outside the per-component damage pipeline) but Shields still absorb it. Stun prevents Initiative gain entirely (0 gain while stacks > 0). **Frozen while Fallen**: status effects do not tick down while a character is in the Fallen state. Revival clears **all** statuses — both beneficial and harmful (true clean slate). All combat values are **integers** with **ceiling rounding** — ensures last stack always decays. See [combat.md](domains/combat/index.md).

### Sneaking
A status that prevents a character from being targeted by single-target or selected-target enemy abilities. AoE effects hit Sneaking characters. Break conditions: using a non-stealth-tagged Action, taking damage (break formula: `break_chance = damage_taken / (stealth_stacks × K)` — linear, transparent). Stealth-tagged Actions can be used without breaking Sneaking. Detection (passive or active) allows targeting but does not remove the status. **Detection is per-turn only** — there is no persistent detection state. Each character must make their own detection check before their turn; teammates who act later must detect independently. Implemented within the standard stack-based status system.

### Search
A default Action (via the Combatant system Trait) for active stealth detection. **Action Speed +50** (costs only 50 Initiative — deliberately fast for map-wide detection). Searches the entire map for Sneaking enemies, with detection chance decreasing by range (same zone = high, adjacent = moderate, distant = low). Detection is **per-turn only** — no persistent state. The searching character can target detected enemies during their turn only; teammates must make their own detection checks. See also: Passive Detection (automatic, same-zone only, per-turn, no action cost).
...
### Defend
A default Action (via the Combatant system Trait) available to all characters. **Action Speed +25** (costs only 75 Initiative). Applies the **Defending** status (1 stack, special decay: clears on turn start). Boosts **both Physical Defense and Magic Defense**, plus **all three Soak families** (Physical, Elemental, Magical) by a fixed percentage (tuning value). Also provides a Stamina recovery burst equal to **Y% of max Stamina pool** (tuning value). **No tick limit** — persists through stun (accepted as thematic: a braced character who gets stunned is harder to kill while helpless; self-stun for indefinite Defend is an accepted emergent strategy with costs). Free Action (no Stamina cost).
...
Within a single Action or Trigger, effect components resolve in type order: (0) Revive effects — resolve first, before all other effects; (1) status/buff/debuff effects; (2) damage/healing effects; (3) movement effects. Components within the same category resolve simultaneously. Revive at Step 0 ensures hybrid revive+buff+heal Actions work: Revive clears all statuses (clean slate), then fresh buffs apply, then heals apply. Defined authoritatively by the [combat](domains/combat/index.md) spec. Multi-Trigger resolution remains simultaneous (all qualifying Triggers fire together).
...
### Resistance (Combat)
A per-damage/status-type defensive value. Status effects always apply (no binary resist roll). Resistance provides dual reduction using **percentage with diminishing returns**: `value × (1 - resist_ratio)` where `resist_ratio = Resistance / (Resistance + K)`. Same formula and same K per type for both **stack reduction** and **damage reduction**. Parallels the Soak formula. **Minimum 1 stack** always applies regardless of Resistance. **No true immunity**: there are no immunity Perks — defense against powerful statuses comes from high Resistance (reduces to 1 stack) + fast attribute-derived decay. **Beneficial effects skip resistance**: friendly buffs and heals always apply at full strength; resistance applies only to harmful effects (including friendly fire from AoE). Sources: Attributes, Perks, equipment, status effects.
...
### Block Chance
A defensive mechanic provided by equipped **shield items** (Buckler, Kite Shield, Tower Shield). Separate from the Shield HP-pool combat mechanic (which is Perk/spell-only). Positioned at **step 5** of the 9-step per-component damage pipeline (after hit bonus, before Soak). On a successful hit, the shield's block chance % triggers; on success, the shield's **block value** (flat amount, quality-scaled) is subtracted from damage. **Blocks all damage types** (physical, elemental, magical). Block value splits **proportionally** across multi-component damage based on relative magnitudes. One block check per Action. Two stats: block chance % (~20% Buckler, ~35% Kite, ~50% Tower) and block value. See [equipment.md](domains/equipment/index.md) and [combat.md](domains/combat/index.md).
...
_Last updated: 2026-02-22 — Combat rounds 26-31: Tick (5-step resolution with Kill Triggers), Action Speed (default Action Speeds), Defend (Action Speed +25, self-stun strategy), Search (Action Speed +50, per-turn detection), Status Effect (ceiling decay example fixed, DoTs bypass Block, true clean slate), Trigger (timing categories, 7 new events, OnOverkill removed, deferred kill model), Effect Resolution Order (Step 0 Revive), Overkill (OnKill payload), Sneaking (per-turn detection), Dying Blow (deferred kill note), Fallen (true clean slate, Step 5 triggers). Previous: 2026-02-22 combat rounds 20-25. 2026-02-21 equipment round 31 cleanup. 2026-02-18 post-combat and combat terms._
